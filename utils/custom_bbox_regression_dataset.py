...
from utils.utils_func import iou, parse_car_csv
...
class BBoxRegressionDataset(Dataset):
    ...
    def __init__(self, root_dir, transform=None):
        ...
        for i in range(len(samples)):
            ...
            jpeg_path = os.path.join(
                root_dir, 'JPEGImages', sample_name + '.jpg')
            bndbox_path = os.path.join(
                root_dir, 'bndboxs', sample_name + '.csv')
            positive_path = os.path.join(
                root_dir, 'positive', sample_name + '.csv')

            jpeg_list.append(cv2.imread(jpeg_path))
            bndboxes = np.loadtxt(bndbox_path, dtype=np.int_, delimiter=' ')
            positives = np.loadtxt(positive_path, dtype=np.int_, delimiter=' ')
    ...
    def __getitem__(self, index: int):
        assert index < self.__len__(), '数据集大小为%d，当前输入下标为%d' % (
            self.__len__(), index)
...
def test2():
    ...
    data_loader = DataLoader(
        data_set, batch_size=128, shuffle=True, num_workers=8)

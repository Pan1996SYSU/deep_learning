    ...
    def __init__(self, root_dir, transform=None):
        ...
        for idx in range(len(samples)):
            ...
            positive_annotations = np.loadtxt(
                positive_annotation_path, dtype=np.int_, delimiter=' ')
            # 考虑csv文件为空或者仅包含单个标注框
            ...
            negative_annotations = np.loadtxt(
                negative_annotation_path, dtype=np.int_, delimiter=' ')
            # 考虑csv文件为空或者仅包含单个标注框
...
def test3():
    ...
    data_loader = DataLoader(
        train_data_set, batch_size=128, num_workers=8, drop_last=True)

...
# 正样本边界框数目：37222
from utils.utils_func import make_dirs, parse_car_csv, parse_xml, iou
...
if __name__ == '__main__':
    ...
    classifier_car_train_dir = '../DATA/voc_car/classifier_car/train'
    # positive
    positive_annotation_dir = os.path.join(classifier_car_train_dir, 'Annotations')

    dst_root_dir = '../DATA/voc_car/bbox_regression/'
    ...
    make_dirs(Path(dst_bndbox_dir).parent)
    make_dirs(Path(dst_positive_dir).parent)
    ...
    for sample_name in samples:
        ...
        positive_bndboxes = np.loadtxt(positive_annotation_path, dtype=np.int_, delimiter=' ')
        # 提取标注边界框
        gt_annotation_path = os.path.join(gt_annotation_dir, sample_name + '.xml')
        bndboxs = parse_xml(gt_annotation_path)
        bndboxs = np.array(bndboxs['car'])
